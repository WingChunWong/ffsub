...
impl AppState {
	...
	/// 锁定错误处理的宏，避免代码重复
	fn with_lock<F, T>(&self, mutex: &Mutex<T>, op: F) -> Result<T, String>
	where
		F: FnOnce(&mut T) -> T,
		T: Copy,
	{
		let mut guard = mutex.lock().map_err(|e| format!("锁竞争错误: {e}"))?;
		Ok(op(&mut guard))
	}

	pub fn is_running(&self) -> Result<bool, String> {
		self.with_lock(&self.running, |val| *val)
	}
	...
	pub fn set_total_duration(&self, duration: f64) -> Result<(), String> {
		let mut guard = self.total_duration.lock().map_err(|e| format!("锁竞争错误: {e}"))?;
		*guard = duration;
		Ok(())
	}

	pub fn get_total_duration(&self) -> Result<f64, String> {
		self.total_duration.lock().map(|guard| *guard).map_err(|e| format!("锁竞争错误: {e}"))
	}
}
