...
pub fn parse_progress_line(line: &str, total_duration_secs: f64) -> Option<EncodeProgress> {
	...
	let frame = extract_value(line, "frame=").and_then(|v| v.trim().parse().ok()).unwrap_or(0);
	let fps = extract_value(line, "fps=").and_then(|v| v.trim().parse().ok()).unwrap_or(0.0);
	let time = extract_value(line, "time=").map(|v| v.trim().to_string()).unwrap_or_default();
	let speed = extract_value(line, "speed=").map(|v| v.trim().to_string()).unwrap_or_default();
